# app.py
# RAMKAR MFS v2.7.1 - Tam Entegrasyon
# MFS Dashboard + RAMKAR v30.4 Tarama (217 Hisse)
...
<style>
    @media (max-width: 768px) {
        ...
        .stTabs [data-baseweb="tab"] { font-size: 0.9rem !important; padding: 8px !important; }
    }
...
# ================================
# KATILIM ENDEKSİ HİSSELERİ (217 Hisse - Güncel)
# ================================
KATILIM_HISSELERI = [
    "ACSEL", "AHSGY", "AKFYE", "AKSA", "AKYHO", "ALBRK",
    "ALCTL", "ALKA", "ALKIM", "ALKLC", "ALTNY", "ALVES",
    "ANGEN", "ARASE", "ARDYZ", "ASELS", "ATAKP", "ATATP",
    "AVPGY", "AYEN", "BAHKM", "BAKAB", "BANVT", "BASGZ",
    "BEGYO", "BERA", "BIENY", "BIMAS", "BINBN", "BINHO",
    "BMSTL", "BNTAS", "BORSK", "BOSSA", "BRISA", "BRKSN",
    "BRLSM", "BSOKE", "BURCE", "BURVA", "CANTE", "CATES",
    "CELHA", "CEMTS", "CEMZY", "CIMSA", "CMBTN", "COSMO",
    "CVKMD", "CWENE", "DAPGM", "DARDL", "DCTTR", "DENGE",
    "DESPC", "DGATE", "DGNMO", "DMSAS", "DOFER", "DOFRB",
    "DOGUB", "DYOBY", "EBEBK", "EDATA", "EDIP", "EFOR",
    "EGEPO", "EGGUB", "EGPRO", "EKGYO", "EKSUN", "ELITE",
    "ENJSA", "EREGL", "ESCOM", "EUPWR", "EYGYO", "FADE",
    "FONET", "FORMT", "FORTE", "FZLGY", "GEDZA", "GENIL",
    "GENTS", "GEREL", "GESAN", "GLRMK", "GOKNR", "GOLTS",
    "GOODY", "GRSEL", "GRTHO", "GUBRF", "GUNDG", "HATSN",
    "HKTM", "HOROZ", "HRKET", "IDGYO", "IHEVA", "IHLAS",
    "IHLGM", "IHYAY", "IMASM", "INTEM", "ISDMR", "ISSEN",
    "IZFAS", "IZINV", "JANTS", "KARSN", "KATMR", "KBORU",
    "KCAER", "KIMMR", "KLSYN", "KNFRT", "KOCMT", "KONKA",
    "KONTR", "KONYA", "KOPOL", "KOTON", "KRDMA", "KRDMB",
    "KRDMD", "KRGYO", "KRONT", "KRPLS", "KRSTL", "KRVGD",
    "KTLEV", "KUTPO", "KUYAS", "KZBGY", "LKMNH", "LMKDC",
    "LOGO", "MAGEN", "MAKIM", "MARBL", "MAVI", "MEDTR",
    "MEKAG", "MERCN", "MNDRS", "MNDTR", "MOBTL", "MPARK",
    "NETAS", "NTGAZ", "OBAMS", "OBASE", "OFSYM", "ONCSM",
    "ORGE", "OSTIM", "OZRDN", "OZYSR", "PAGYO", "PARSN",
    "PASEU", "PENGD", "PENTA", "PETKM", "PETUN", "PKART",
    "PLTUR", "PNLSN", "POLHO", "QUAGR", "RGYAS", "RNPOL",
    "RODRG", "RUBNS", "SAFKR", "SAMAT", "SANEL", "SANKO",
    "SARKY", "SAYAS", "SEKUR", "SELEC", "SELVA", "SILVR",
    "SMART", "SMRTG", "SNGYO", "SNICA", "SOKE", "SRVGY",
    "SUNTK", "SURGY", "SUWEN", "TARKM", "TDGYO", "TEZOL",
    "TKNSA", "TMSN", "TNZTP", "TUCLK", "TUKAS", "TUPRS",
    "TUREX", "ULAS", "ULUSE", "USAK", "VAKKO", "VANGD",
    "VESBE", "VRGYO", "YATAS", "YEOTK", "YUNSA", "ZEDUR",
    "ZERGY"
]

# ================================
# MFS CONFIG (Dokunulmadı)
# ================================
TH = {
...
# ================================
# RAMKAR TEKNİK ANALİZ FONKSİYONLARI
# ================================
...
def get_xu100_data():
    ...
    try:
        ...
        bist_ok = xu100_close > xu100_ema50
...
def ramkar_scan_single(symbol: str, xu100_data, xu100_close, xu100_ema50) -> Optional[Dict]:
    """Tek hisse RAMKAR taraması"""
    try:
        ...
        return {
            ...
            'di_plus': round(last_di_plus, 1),
            'di_minus': round(last_di_minus, 1),
            'k': round(last_k, 1),
            'd': round(last_d, 1),
...
def run_full_scan(progress_callback=None):
    ...
    xu100_data, xu100_close, xu100_ema50 = get_xu100_data()
    ...
    for i, symbol in enumerate(KATILIM_HISSELERI):
        ...
        result = ramkar_scan_single(symbol, xu100_data, xu100_close, xu100_ema50)
        if result:
    ...
    return results, xu100_close, xu100_ema50


# ================================
# MFS FONKSİYONLARI (Dokunulmadı)
# ================================
def clamp(x, lo, hi):
...
base_pos, base_risk, _ = BASE_BUDGETS[regime]
if soft_reduction > 0:
...
with c2:
    if st.button("🚨", help="Acil Durum"):
        st.session_state.manual_kill = not st.session_state.manual_kill
        st.rerun()
...
# Tabs
tab1, tab2 = st.tabs(["📊 MFS", "🔥 RAMKAR TARAMA"])
...
with tab1:
    ...
    # Karar kutusu
    if regime == "ON":
        st.success(f"🟢 **YEŞİL** | Max {adj_pos} poz, {adj_risk}R | RAMKAR sinyallerini değerlendir")
    elif regime == "NEUTRAL":
...
with tab2:
    ...
    if not YF_AVAILABLE:
        ...
        if scan_btn:
            ...
            results, xu100_c, xu100_e = run_full_scan(update)
        ...
        if st.session_state.scan_results:
            ...
            **🛑 STOP:** {len(stop)}
            ...
            # MFS Uyarısı
            if regime == "OFF-KILL":
                st.error("💀 MFS KİLİTLİ - Tüm sinyaller BLOKE!")
            elif regime == "OFF":
                st.warning("🔴 MFS OFF - Yeni giriş riskli!")
            ...
            for i, r in enumerate(results[:10]):
                blocked = regime in ["OFF-KILL", "OFF"]
                opacity = "0.5" if blocked else "1"
                border = "#666" if blocked else r['status_color']
                
                # Kriterler
                crit = f"{'✅' if r['trend_ok'] else '❌'}{'✅' if r['power_ok'] else '❌'}{'✅' if r['vol_ok'] else '❌'}{'✅' if r['sar_ok'] else '❌'}{'✅' if r['dist_ok'] else '❌'}{'✅' if r['bist_ok'] else '❌'}"
                ...
                with c1:
                    ...
                    <div style="opacity:{opacity};">
                        <span style="font-size:1.2rem;font-weight:bold;color:white;">{i+1}. {r['symbol']}</span><br>
                        <span style="color:#888;">₺{r['price']:.2f}</span>
                    </div>
                    """, unsafe_allow_html=True)
                
                with c2:
                    st.markdown(f"""
                    <div style="opacity:{opacity};font-size:0.85rem;">
                        <span style="color:#ccc;">{crit}</span><br>
                        <span style="color:#666;">ADX:{r['adx']:.0f} MFI:{r['mfi']:.0f} Dist:{r['ema_dist']:+.1f}%</span>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                with c3:
                    if blocked:
                        st.markdown(f"""
                        <div style="background:#333;border-radius:8px;padding:6px;text-align:center;">
                            <span style="color:#666;font-weight:bold;">{r['score']}/6</span><br>
                            <span style="color:#666;font-size:0.8rem;">🚫 BLOKE</span>
                        </div>
                        """, unsafe_allow_html=True)
                    else:
                        st.markdown(f"""
                        <div style="background:{r['status_color']}22;border:2px solid {r['status_color']};border-radius:8px;padding:6px;text-align:center;">
                            <span style="color:{r['status_color']};font-weight:bold;">{r['score']}/6</span><br>
                            <span style="color:{r['status_color']};font-size:0.8rem;">{r['status_icon']} {r['status']}</span>
                        </div>
                        """, unsafe_allow_html=True)
            ...
            st.markdown("""
            **6 Kriter:**
            1. Trend: Fiyat > EMA20, K > D
            2. Güç: ADX ≥ 28, DI+ > DI-
            3. Hacim: ≥ 1.2x ortalama
            4. SAR: Fiyat > Parabolic SAR
            5. Mesafe: -2% ile +30% arası
            6. BIST: XU100 > EMA50
            """)
